...
foreach ($test as $row) {
    ...
    $master->templateName = $root . "/frontend/templates/car_rss.tpl";
    $product_id = $row['id'];
...
function get_img($car_id)
{
    ...
    foreach ($test as $row) {
        return ['https://www.dompereezdov.ru/sync/' . $row['small_img'], 'https://www.dompereezdov.ru/sync/' . $row['big_img']];
    }
}
