...
function add_pagination($sql_query, $limit, $link, $page, $additional_params) {
// Функция создает строку с постраничной навигацией. Например: Страница 1 2 3 4
...
// Полученное количество делим на доступный лимит и округляем в большую сторону
    ...
    if ($page > 1) {
        ...
        $href_left = ' href="/?link=' . $link . '&page=' . ($page - 1) . $ap . '" ';
    }
    ...
    while ($i <= $page_count) {
        ...
        if ($i == $page) {
            ...
            $additional_params != '' ? $ap = $additional_params : $ap = '';
            $html .= '<li><a href="/?link=' . $link . '&page=' . $i . $ap . '">' . $i . '</a></li>';
        }
        $i++;
    }
    ...
    if ($page < $page_count) {
        $additional_params != '' ? $ap = $additional_params : $ap = '';
        $href_right = ' href="/?link=' . $link . '&page=' . ($page + 1) . $ap . '" ';
    }
